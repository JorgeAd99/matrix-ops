...
export default function ChangelogBanner() {
  ...
  // Show modal after a slight delay for smooth entry on first visit
  useEffect(() => {
    const isDismissed = localStorage.getItem('changelog-v2-dismissed');
    if (!isDismissed) {
  ...
  const dismissBanner = () => {
    setIsVisible(false);
    localStorage.setItem('changelog-v2-dismissed', 'true');
  };
